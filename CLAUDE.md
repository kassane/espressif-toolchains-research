...
experiments/zero-cost/    run.sh {esp32|esp32s2|esp32s3|esp32c3|esp32p4} — D × C++ × Rust × C zero-cost abstraction parity: monomorphization, lambdas/Fn-traits, static vs dynamic dispatch, heap-allocated classes. Parameterized across xtensa (esp32 baseline) AND riscv (esp32c3 / esp32p4); ISA-portable conclusion (docs/25 + docs/27 §a)
experiments/tmp-parity/   run.sh {esp32|...|esp32p4} — D × C++ × Rust template-metaprogramming feature surface comparison (12 capabilities × 3 languages); covers parameter forms, constraints, specialization, static-if, mixin, CTFE, introspection, variadic. Also disentangles D's three extern(C++) FFI forms (producer class / consumer class struct / POD struct) with llvm-nm symbol-role evidence. Parameterized across xtensa + riscv; docs/26 + docs/27 §b
experiments/simd/         run.sh — xtensa esp32s3 EE.* PIE (4 frontends) PLUS §7 esp32p4 vendor riscv ESPV (xespv1v 2.1 / xespv 2.2, mnemonic family esp.*, q0..q? regs, 128-bit). Cross-frontend byte-identical asm encodings via shared libLLVM RISC-V assembler. Recommended -mcpu=esp32p4eco4 (ESPV 2.1 = 412 documented mnemonics); see docs/16 + docs/27 §c
scripts/                  setup.sh env.sh build-ffi.sh analyze.sh run-qemu.sh
docs/00..27               support-matrix / toolchains / abi / … / dlang-ldc / dlang-safety / tmp-ffi-baremetal / dwarf-codegen-parity / ldc-espressif-fork / tinygo / zero-cost / tmp-parity / riscv-parity
