...
for fn in add_i32 mul_f64 make_point point_dot make_blob blob_sum apply; do
  echo "== $fn =="; for l in c rs zig; do printf "  %-4s " "$l"; grep -E "define.*$fn" "$IR/lib_$l.ll" | head -1; done
...
for fn in c_add_i32 rs_add_i32 zig_add_i32 c_apply zig_apply c_blob_sum zig_blob_sum c_make_blob zig_make_blob; do
  echo "== $fn =="
...
for o in lib_c_clang lib_c_gcc lib_cpp lib_zig; do
